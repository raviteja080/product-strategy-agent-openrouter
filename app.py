...
def post_openrouter_with_retry(api_key, json_payload, timeout=120, max_retries=5):
    headers = {
        "Authorization": f"Bearer {api_key}",
        "HTTP-Referer": "https://strategy-agent.netlify.app",
        "X-Title": "Product Strategy Agent",
...
def call_openrouter(api_key, instruction, prompt):
    json_payload = {
    ...
    result = post_openrouter_with_retry(api_key, json_payload)
    
    raw_text = result.get("choices", [{}])[0].get("message", {}).get("content", "")
            
    sources = [] # Note: Grounding metadata is not natively available like Gemini
            
    if not raw_text:
        raise Exception("No response from OpenRouter.")
        
    parsed_json = json.loads(extract_json_from_llm(raw_text))
...
def analyse():
    ...
    try:
        ...
        if analysis_type in ["strategy", "both"]:
            strategy_json, sources = call_openrouter(api_key, SYSTEM_INSTRUCTION, f"Research the product '{product}' and its top competitors thoroughly. FIND unique sources for each competitor.")
        ...
        if analysis_type in ["pricing", "both"]:
            ...
            prompt = f"Research the pricing of the product '{product}' AND its active competitors thoroughly. ENSURE you retrieve distinct, unique URLs for each competitor you list."
            if analysis_type == "both":
                prompt += f"\n\nHere is the Product Strategy context to use logically:\n{json.dumps(final_result)}"
            
            pricing_json, sources = call_openrouter(api_key, PRICING_INSTRUCTION, prompt)
...
Use real researched data wherever possible."""
    ...
    try:
        json_payload = {
            "model": "meta-llama/llama-3.3-70b-instruct",
            "messages": [
               {"role": "system", "content": comp_inst},
               {"role": "user", "content": f"Research products '{p1}' and '{p2}' thoroughly."}
            ],
            "temperature": 0.3
        }
        res_json = post_openrouter_with_retry(api_key, json_payload, timeout=150)
        ...
        parsed = json.loads(extract_json_from_llm(raw_text))
